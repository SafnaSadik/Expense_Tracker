...
class EnhancedGitHubManager:
    ...
    def save_extracted_content(self):
        """Save extracted HTML, CSS, and Python code to local files"""
        ...
        for file in html_files:
            ...
            dest_file = os.path.join("exported_templates", file)
    ...
    def upload_project_files(self):
        ...
        try:
            folders = ["exported_templates", "exported_static", "python_code"]
            for folder in folders:
                for file in os.listdir(folder):
                    ...
                    repo_path = f"{folder}/{file}"
                    try:
                        existing_file = self.repo.get_contents(repo_path)
                        self.repo.update_file(repo_path, f"Updated {file}", content, existing_file.sha)
                    except:
                        self.repo.create_file(repo_path, f"Added {file}", content)
...
def main():
    ...
    if manager.initialize_github():
        manager.save_extracted_content()
