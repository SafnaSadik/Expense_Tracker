...
class EnhancedGitHubManager:
    ...
    def extract_content(self, notebook_path):
        ...
        try:
            ...
            self.save_extracted_files(content)
    ...
    def save_extracted_content(self, content):
        """
        Save extracted content to appropriate folders before committing to GitHub
        """
        os.makedirs("exported_templates", exist_ok=True)
        os.makedirs("exported_static", exist_ok=True)
        os.makedirs("python_code", exist_ok=True)

        if content['html']:
            with open("exported_templates/base.html", "w", encoding="utf-8") as f:
                f.write("\n".join(content['html']))

        if content['html']:
            with open("exported_templates/dashboard.html", "w", encoding="utf-8") as f:
                f.write("\n".join(content['html']))

        if content['html']:
            with open("exported_templates/login.html", "w", encoding="utf-8") as f:
                f.write("\n".join(content['html']))

        if content['html']:
            with open("exported_templates/register.html", "w", encoding="utf-8") as f:
                f.write("\n".join(content['html']))

        if content['css']:
            with open("exported_static/styles.css", "w", encoding="utf-8") as f:
                f.write("\n".join(content['css']))

        if content['python']:
            with open("python_code/main.py", "w", encoding="utf-8") as f:
                f.write("\n".join(content['python']))

        print("Extracted files saved successfully!")
...
def main():
    """Main function to extract and upload project files"""
    manager = EnhancedGitHubManager()

    if manager.initialize_github():
        # Extract content from the notebook
        extracted_content = manager.extract_content("/content/drive/MyDrive/Colab Notebooks/Final Project - Expense Tracker.ipynb")

        if extracted_content:
            # Save extracted content to respective files
            manager.save_extracted_content(extracted_content)

            # Upload the extracted files to GitHub
            if manager.upload_project_files():
                # List all versions after successful upload
                manager.list_versions()
        else:
            print("Failed to extract content from notebook")
    else:
