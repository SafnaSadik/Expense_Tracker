...
def main():
    """Main function to create and upload multiple versions"""
    manager = EnhancedGitHubManager()

    if manager.initialize_github():
        # Create and upload 5 versions
        for version_number in range(1, 6):
            print(f"\nProcessing Version {version_number}")
            print("-" * 30)

            # Save content to version-specific folder
            version_folder = manager.save_extracted_content(version_number)

            # Upload the version folder
            if manager.upload_version_folder(version_folder):
                print(f"Successfully uploaded version {version_number}")
            else:
                print(f"Failed to upload version {version_number}")
